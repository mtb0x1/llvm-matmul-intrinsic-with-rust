use crate::common::GPU_PTX_PAYLOAD;
...
type KernelKey = (ShapeKey, &'static str); // shape and kernel name
...
impl GpuKernelCache {
    ...
    fn get_or_load(
        ...
        // Compile/load module and kernel
        let mut module: CUmodule = ptr::null_mut();
        let ptx_cstring = CString::new(ptx).expect("Failed to create CString from PTX");
        unsafe {
            check_cuda_error(
...
pub unsafe fn ll_matmul(
    ...
    unsafe {
        ...
        check_cuda_error(cuCtxSetCurrent(ctx.jitcontext), "cuCtxSetCurrent for jit");
        ...
        let entry = cache.get_or_load(shape_key, kernel_name, GPU_PTX_PAYLOAD);
        ...
        // eprintln!("Launching kernel {:?} with:", entry.function);
        // eprintln!("device_ptr_a: {:?}", device_ptr_a);
        // eprintln!("device_ptr_b: {:?}", device_ptr_b);
        // eprintln!("device_ptr_c: {:?}", device_ptr_c);
        // eprintln!("m: {}, n: {}, k: {}", m, n, k);
        // eprintln!(
        //     "grid: ({}, {}), block: ({}, {})",
        ...
        check_cuda_error(
            cuMemcpyDtoH_v2(result.as_mut_ptr() as *mut _, device_ptr_c, vec_c_size * 4),
